-- Saint v1 | Init Bootstrap — Free Version
-- Author: hogoromoeton67
...
local function fireLoadNotifications()
    ...
    if env.catrole == 'HWID MISMATCH' then
        vape:CreateNotification('SAINT', 'HWID MISMATCH — go to the script panel to reset HWID', 25, 'alert')
        env.catrole = ''
        task.wait(0.1)
    end

    if not shared.vapereload then
        local authStr = env.catname and ('Authenticated as %s with %s, '):format(env.catname, env.catrole) or ''
        local openStr = vape.VapeButton and 'Press the button in the top right' or ('Press %s to open GUI'):format(table.concat(vape.Keybind, ' + '):upper())

        vape:CreateNotification('Finished Loading', authStr .. openStr, 5)

        task.delay(0.05 + RunService.PostSimulation:Wait(), function()
            if shared.updated then
                vape:CreateNotification('SAINT', ('Script updated from %s to main'):format(shared.updated), 10, 'info')
            end
        end)
    end
end
...
do
    ...
    vape = loadstring(downloadFile(CACHE_ROOT .. 'guis/' .. guiVariant .. '.lua'), 'gui')(license)
...
if not shared.VapeIndependent then
    ...
    elseif not shared.VapeDeveloper then
        local ok, data = pcall(game.HttpGet, game, REMOTE_ROOT .. 'games/' .. game.PlaceId .. '.lua', true)
